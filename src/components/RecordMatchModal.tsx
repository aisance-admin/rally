  ...
  return (
    <Modal onClose={onClose}>
      ...
      <div className="space-y-4 px-5 py-5">
          ...
          <div className="flex gap-1.5">
            {FORMATS.map((f) => (
              <button
                ...
                style={
                  format === f
                    ? { background: '#ff550022', color: '#ff7a33', boxShadow: 'inset 0 0 0 1px #ff550066' }
        ...
        {preview && a && b && (
          <div className="rounded-lg bg-ink-800 px-3 py-2.5 text-sm ring-1 ring-ink-700">
      ...
      <div className="flex gap-2 border-t border-ink-800 px-5 py-4">
        <button onClick={onClose} className="flex-1 rounded-lg bg-ink-800 py-2.5 text-sm font-semibold text-ink-500 hover:text-white">
          Cancel
        </button>
        <button
          onClick={submit}
          disabled={sameError || tie}
          className="flex-1 rounded-lg bg-brand py-2.5 text-sm font-bold text-ink-900 hover:bg-brand-400 disabled:opacity-40"
        >
          Save & update ratings
        </button>
      </div>
...
function PlayerPick({
  ...
  return (
    <div
      ...
      style={{
        background: highlight ? '#32d74b14' : '#13171e',
        boxShadow: highlight ? 'inset 0 0 0 1px #32d74b55' : 'inset 0 0 0 1px #1d2430',
      }}
    >
      ...
      <select
        ...
        className="w-full rounded-lg bg-ink-900 px-2 py-2 text-sm font-semibold outline-none ring-1 ring-ink-700"
      >
        {players.map((p) => (
  ...
  return (
    <div className="flex items-center justify-center gap-2">
      <button
        ...
        className="grid h-9 w-9 place-items-center rounded-lg bg-ink-800 text-lg font-bold text-ink-500 hover:text-white"
      >
        −
      </button>
      <input
        ...
        className="w-14 rounded-lg bg-ink-900 py-2 text-center font-mono text-xl font-extrabold outline-none ring-1 ring-ink-700"
      />
      <button
